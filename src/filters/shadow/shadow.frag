...
void main(void) {
    ...
    float light = min(
      1.0 - length(texture2D(uSampler, vTextureCoord) - texture2D(uSampler, dl)),
      texture2D(uSampler, dl).a
    );
    ...
    float shadow = min(
      1.0 - length(texture2D(uSampler, vTextureCoord) - texture2D(uSampler, ds)),
      texture2D(uSampler, ds).a
    );
